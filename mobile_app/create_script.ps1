$p = 'C:\Users\marco\Xubudget\Xubudget\mobile_app\RUN_XUBUDGET_ALL_v3.cmd'
$s = @'
@echo off
setlocal
REM WHY: levantar tudo na ordem e evitar cair na URL de debug
set ROOT=C:\Users\marco\Xubudget\Xubudget
set BACK=%ROOT%\services\pi2_assistant
set FRONT=%ROOT%\mobile_app
set PORT=5210

REM Fecha Chrome pra não herdar aba de debug
...
start "Xubudget Backend" cmd /k "cd /d %BACK% && uvicorn pi2_server:app --host 127.0.0.1 --port 5001 --reload"
...
REM Descobre flutter no PATH; se não tiver, usa caminho padrão
set "FLUTTER=flutter"
...
start "Xubudget Frontend" cmd /k "cd /d %FRONT% && \"%FLUTTER%\" run -d chrome --web-hostname=localhost --web-port=%PORT%"
...
REM Agora abre o APP (não a página de debug)
