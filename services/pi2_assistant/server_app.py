...
def process_chat_command(message: str) -> ChatResponse:
    ...
    # Detectar gastos
    if any(word in message_lower for word in ["gastei", "gasto", "comprei", "paguei"]):
        # Extrair valor
        import re
...
async def get_dashboard_summary(user_id: str = "default"):
    update_state(user_id)
    
    total_budget = sum(icon["budget"] for icon in user_state["icons"].values())
    total_spent = sum(icon["spent"] for icon in user_state["icons"].values())
    available_amount = total_budget - total_spent
    
    # Separar categorias principais e secundárias
    icons_list = list(user_state["icons"].values())
    icons_list.sort(key=lambda x: x["spent"], reverse=True)
    
    primary_categories = icons_list[:4]  # Top 4
    secondary_categories = icons_list[4:]  # Resto
    
    # Gerar insights
    insights = []
    for icon in primary_categories:
        percentage = (icon["spent"] / icon["budget"]) * 100 if icon["budget"] > 0 else 0
        
        if percentage > 100:
            insights.append({
                "type": "warning",
                "message": f"{icon['name']} passou do limite.",
                "icon": "AlertCircle"
            })
        elif percentage < 50:
            insights.append({
                "type": "success", 
                "message": f"{icon['name']} sob controle este mês!",
                "icon": "CheckCircle2"
            })

    # Cálculo do daily_pace e daily_delta_pct
    now = datetime.now()
    days_in_month = calendar.monthrange(now.year, now.month)[1]
    days_remaining = (datetime(now.year, now.month, days_in_month) - now).days + 1
    days_passed = days_in_month - days_remaining

    daily_pace = (total_budget - total_spent) / days_remaining if days_remaining > 0 else 0

    actual_avg_daily_spent = total_spent / days_passed if days_passed > 0 else 0
    target_avg_daily_spent = total_budget / days_in_month if days_in_month > 0 else 0
    
    daily_delta_pct = ((actual_avg_daily_spent - target_avg_daily_spent) / target_avg_daily_spent) * 100 if target_avg_daily_spent > 0 else 0
